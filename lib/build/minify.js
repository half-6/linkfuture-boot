...
async function minifyJS(inputList,output,minify) {
    // JS
    //src/static/js/output/front-global.v1.min.js
    //src\static\js\output\front-global.v1.min.js
    //minify or just merge
    console.log(`minify ${output}`);
    await new Promise((resolve, reject)=>{
        $compressor.minify({
            compressor: minify?'uglifyjs':'no-compress',
            //publicFolder: $config.config.jsFolder,
            input: inputList,
            output: output,
...
async function minifyCSS(inputList,output,minify) {
    console.log(`minify ${output}`);
    await new Promise((resolve, reject)=>{
        $compressor.minify({
            compressor: minify?'clean-css':'no-compress',
            //publicFolder: $config.config.cssFolder,
            input: inputList,
            output: output,
            callback: function (err, min) {
                if(err){
...
async function minify(config,type){
    switch (type)
    {
        case buildType.js:
            await minifyJS(config.files,config.output,true);
            break;
        case buildType.css:
            await minifyCSS(config.files,config.output,true);
            break;
        case buildType.sql:
...
async function build(buildFile,type) {
