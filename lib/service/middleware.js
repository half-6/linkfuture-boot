  ...
  res.send = body => {
    ...
    if (res.statusCode >= 200 && res.statusCode < 300) {
      const output = meta.api
        ...
        : body;
      if (meta.store) {
        $lf.$cache.factory.set(
          key,
          { type: res.get('Content-Type'), output },
          meta.min
        );
      }
...
async function bind(input, req, res, next) {
  ...
  const cachedObj = await $lf.$cache.factory.get(key);
  if (cachedObj) {
    if (cachedObj.type) res.type(cachedObj.type);
    res.send(cachedObj.output);
  } else {
