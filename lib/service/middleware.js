...
const $securityMapping = $lf.$util.readJson(`${$config.resourceroot}security.json`);
const $defaultMeta = {
    min:0,
    key: function (req) {
...
function bind(input,req, res, next) {
    ...
    //authenticate
    if((meta.auth || $lf.$auth.isUrlMatch( $securityMapping,req.getUrl) ) && !req.isAuthenticated())
    {
