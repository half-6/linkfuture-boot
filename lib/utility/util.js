...
/**
 * read .env file from resource folder and inject into env
 * @param envFile
 */
function readEnvFile(envFile) {
  if ($fs.existsSync(envFile)) {
    $debugger(`read env file ${envFile}`);
    const fileString = $fs.readFileSync(envFile).toString();
    fileString.split($newLine).forEach(line => {
      const envObj = line.split('=');
      if (envObj.length === 2) {
        process.env[envObj[0].trim()] = envObj[1].trim();
      }
    });
  }
}
