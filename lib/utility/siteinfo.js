...
const $meta = _.get($config,"config.static",$defaultMeta);
$assert($meta,"missing static setting");

function getJsBuild() {
    return $cache.upsetStatic("jsBuildFile",()=>{
        return  $util.readJson($meta.jsBuildFile);
    });
}
function getCssBuild() {
    return $cache.upsetStatic("cssBuildFile",()=>{
...
function buildJSFiles(name) {
    ...
    return $cache.upsetStatic("JS-Reference-Files-" + name + "-" +$meta.minify,function () {
        if($jsBuild[name])
        {
...
function buildJSInline(name) {
    return $cache.upsetStatic("JS-Reference-Inline-" + name,function () {
...
function buildJSReference(name,isUnblock) {
    return $cache.upsetStatic("JS-Reference-" + name + "-" + isUnblock,function () {
...
function buildCSSReference(name) {
    ...
    return $cache.upsetStatic("CSS-Reference-" + name,function () {
        if ($cssBuild[name]) {
